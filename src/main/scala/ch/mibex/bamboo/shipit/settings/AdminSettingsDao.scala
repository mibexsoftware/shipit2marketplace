...
class AdminSettingsDao @Autowired()(@ComponentImport ao: ActiveObjects,
  ...
  def createOrUpdate(vendorName: String, vendorPassword: String): AoAdminSettings =
    ao.executeInTransaction(new TransactionCallback[AoAdminSettings]() {
      override def doInTransaction(): AoAdminSettings = {
  ...
  def find(): Option[AoAdminSettings] =
    ao.executeInTransaction(new TransactionCallback[Option[AoAdminSettings]]() {
      override def doInTransaction(): Option[AoAdminSettings] =
        ao.find[AoAdminSettings, Integer](classOf[AoAdminSettings]).headOption
    })
